...
export default function OgTab({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="p-6">
      ...
      {taskId && (
        <div className={cx('rounded-xl p-3 mb-4 text-sm', isDone ? 'bg-green-50 border border-green-200 text-green-700' : isRunning ? 'bg-blue-50 border border-blue-200 text-blue-700' : 'bg-red-50 text-red-700')}>
        ...
        data?.crawl_status === 'not_done' ? (
          ...
          <div className="space-y-2">
            {(data?.pages ?? []).map((page: OgPage) => {
              ...
              return (
                <div key={page.page_url} className={cx('border border-[var(--border)] rounded-xl bg-surface overflow-hidden',
                  ...
                  {isExpanded && (
                    <div className="border-t p-3 bg-surface-raised space-y-3">
                      {/* Current OG */}
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          <p className="text-muted font-medium mb-1">og:title</p>
                          <p className={cx(page.missing_title ? 'text-red-500 italic' : 'text-primary')}>{page.og_title || 'нет'}</p>
                        </div>
                        <div>
                          <p className="text-muted font-medium mb-1">og:description</p>
                          <p className={cx(page.missing_description ? 'text-red-500 italic' : 'text-primary')}>{page.og_description || 'нет'}</p>
                        </div>
                        <div>
                          <p className="text-muted font-medium mb-1">og:image</p>
                          <p className={cx(page.missing_image ? 'text-red-500 italic' : 'text-primary truncate')}>{page.og_image || 'нет'}</p>
                        </div>
                        <div>
                          <p className="text-muted font-medium mb-1">og:type</p>
                          <p className="text-primary">{page.og_type || 'нет'}</p>
                        </div>
                      </div>
                      ...
                      {(page.og_title || form.rec_og_title) && (
                        <div>
                            ...
                            {(['telegram', 'vk', 'whatsapp'] as const).map((p) => (
                              <button key={p} onClick={() => setPreviewPlatform(p)}
                                className={cx('text-xs px-2.5 py-1 rounded-full border font-medium transition',
                                  previewPlatform === p ? 'bg-accent text-white border-accent' : 'bg-surface text-muted hover:bg-surface-raised')}>
                          ...
                          {previewPlatform === 'telegram' && (
                            <div className="border-l-4 border-blue-400 bg-[#eef3fb] rounded-r-lg overflow-hidden max-w-sm">
                              ...
                              <div className="p-2">
                                ...
                                <p className="text-sm font-semibold text-primary line-clamp-2">{form.rec_og_title || page.og_title}</p>
                                <p className="text-xs text-muted line-clamp-2 mt-0.5">{form.rec_og_description || page.og_description}</p>
                              </div>
                            </div>
                          )}
                          ...
                          {/* VK */}
                          {previewPlatform === 'vk' && (
                            <div className="border rounded-xl overflow-hidden bg-surface max-w-sm shadow-sm">
                              ...
                              <div className="p-3 border-t">
                                <p className="text-xs text-muted uppercase tracking-wide mb-1">{new URL(page.page_url).hostname}</p>
                                <p className="text-sm font-bold text-primary line-clamp-2">{form.rec_og_title || page.og_title}</p>
                                <p className="text-xs text-muted line-clamp-3 mt-1">{form.rec_og_description || page.og_description}</p>
                              </div>
                            </div>
                          )}
                          ...
                          {previewPlatform === 'whatsapp' && (
                            <div className="bg-[#dcf8c6] rounded-xl overflow-hidden max-w-sm p-0.5">
                      ...
                      <div className="space-y-2 pt-1">
                        <div>
                          <div className="flex justify-between mb-0.5">
                            <label className="text-xs text-muted">Рек. og:title</label>
                        ...
                        <div>
                          <div className="flex justify-between mb-0.5">
                            <label className="text-xs text-muted">Рек. og:description</label>
                        ...
                        <div className="pt-1 border-t">
                          <p className="text-xs font-semibold text-muted mb-1.5">Twitter Card</p>
                          <div className="flex gap-2 mb-1.5">
                            {(['summary', 'summary_large_image'] as const).map((v) => (
                              <button key={v} type="button"
                                onClick={() => setEditForms((f) => ({ ...f, [page.page_url]: { ...form, twitter_card: v } }))}
                                className={cx('text-xs px-2 py-1 rounded border', form.twitter_card === v ? 'bg-accent text-white border-accent' : 'bg-surface text-muted')}>
                                {v === 'summary' ? 'summary' : 'summary_large_image'}
                              </button>
                            ))}
                          </div>
                          <input className="w-full border rounded px-2 py-1 text-sm bg-surface mb-1"
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setExpandedUrl(null)} className="border px-3 py-1.5 rounded-xl text-sm hover:bg-surface">Закрыть</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {(data?.pages ?? []).length === 0 && (
              <div className="text-center py-10 text-muted">Нет страниц с OG проблемами 🎉</div>
            )}
          </div>
        )
      }
      ...
      <div className="mt-6 border-t pt-4">
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-semibold text-sm text-primary">Экспорт HTML-кода OG тегов</h4>
          <button
        ...
        {showHtmlExport && (
          <div className="space-y-2">
            {!htmlSnippets ? (
              ...
              <p className="text-sm text-muted">Нет страниц с рекомендациями. Сначала сгенерируйте OG теги.</p>
            ) : (
              <>
                <p className="text-xs text-muted mb-2">{htmlSnippets.total} страниц с рекомендациями</p>
                {htmlSnippets.snippets.map((s) => (
                  <div key={s.page_url} className="border border-[var(--border)] rounded-xl bg-surface overflow-hidden">
                    <div className="flex items-center justify-between px-3 py-2 bg-surface-raised border-b">
                      <span className="text-xs font-mono text-muted truncate flex-1">{s.page_url}</span>
                      <button
                        onClick={() => copySnippet(s.page_url, s.html)}
                        className={cx('text-xs px-2.5 py-1 rounded ml-2 shrink-0 transition font-medium',
                          copiedUrl === s.page_url ? 'bg-green-100 text-green-700' : 'bg-accent-subtle text-accent hover:bg-accent-subtle')}
                      >
                        {copiedUrl === s.page_url ? '✅ Скопировано' : '📋 Скопировать'}
                      </button>
                    </div>
                    <pre className="text-xs text-primary p-3 overflow-x-auto bg-surface-raised font-mono leading-relaxed">{s.html}</pre>
                  </div>
                ))}
              </>
            )}
