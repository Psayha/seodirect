...
def upgrade() -> None:
    ...
    if "project_images" not in inspector.get_table_names():
        op.create_table(
            "project_images",
    ...
    if "ix_project_images_project_id" not in existing_indexes:
        op.create_index("ix_project_images_project_id", "project_images", ["project_id"])
    if "ix_project_images_created_at" not in existing_indexes:
