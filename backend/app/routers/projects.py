...
async def brief_chat(
    ...
    prompt_template = get_prompt("brief_chat", db) or (
        "Ты — опытный специалист по поисковому маркетингу. Помогаешь заполнить и улучшить бриф для проекта."
    )
    system_prompt = (
        f"{prompt_template}\n\n"
        f"Проект: «{project.name}» (сайт: {project.url})\n\n"
...
async def brief_improve(
    ...
    system_prompt = get_prompt("brief_improve", db) or (
        "Ты — эксперт по поисковому маркетингу. Проанализируй бриф и верни улучшенные значения полей в JSON."
    )
